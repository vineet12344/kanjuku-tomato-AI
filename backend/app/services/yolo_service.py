...
def _postprocess_and_annotate(results, original_image: Image.Image, conf_threshold=0.25):
    """
    Postprocess YOLO results: draw bounding boxes, label them, and return detections + base64 image.
    """
    detections = []
    img_draw = original_image.copy()
    draw = ImageDraw.Draw(img_draw)
    ...
    try:
        font = ImageFont.truetype("arial.ttf", 88)  # Larger font for clarity
    except Exception:
        font = ImageFont.load_default()
    ...
    for (box, conf, cls) in zip(xyxy, confs, cls_idxs):
        ...
        color = "green" if label.lower() == "ripe" else "blue"
        ...
        text_origin = (x1, y1 - text_size[1] - 4) if (y1 - text_size[1] > 0) else (x1, y1 + 4)

        # Draw filled background for text
        draw.rectangle(
            [text_origin, (text_origin[0] + text_size[0] + 4, text_origin[1] + text_size[1] + 4)],
            fill=color
        )
